...
resource "aws_iam_role_policy" "input-dataset2-access" {
    count = var.cdh_input2_dataset_name != "" ? 1 : 0

    name = "${var.lambda_function_name}-input-dataset2-access"
    role = aws_iam_role.lambda.name
    policy = templatefile(
        "${path.module}/policies/input_dataset_access.json", {
            aws_account_id = var.aws_account_id
            input_s3_bucket = var.cdh_input2_s3_bucket 
            input_kms_key_arn = var.cdh_input2_s3_kms_key_arn
            input_catalog_aws_account_id = var.cdh_input2_glue_catalog_aws_account_id
            input_database = var.cdh_input2_database_name
        }
    )
}
